...
usage: test function
protocol: http://sq.kb.cn/kbsq/wiki/docs/api/tfsproxy
...
def test():
    ...
    origin_name = "T1hRJTByhT1RCvBVdK"
    origin_content = "asdf"

    tfs = TFS('172.16.10.201', 4000, app_id, app_key)

    print 'check_file:', tfs.check_file(origin_name)

    print 'stating upload'
    tfs.start_upload(origin_name, len(origin_content))
    tfs.write_chunk(origin_content)
    print 'writing content: ', len(origin_content)
    tfs_name = tfs.finish_upload()
    print 'tfs filename:', tfs_name
    print 'tfs filepath', tfs.get_file_path(tfs_name)
    print 'check file:', tfs.check_file(tfs_name)

    print 'delete:', tfs.delete_file(tfs_name)
    print 'check file:', tfs.check_file(tfs_name)


    print 'asdf', tfs.put('asdf.jpg', 'asdf')
